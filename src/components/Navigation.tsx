...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled 
        ? 'glass backdrop-blur-xl border-b border-white/40' 
        : 'bg-transparent'
    }`}>
      ...
      {isOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 glass-strong backdrop-blur-xl border-b border-white/40">
            {navItems.map((item) => (
              <button
                key={item.label}
                onClick={() => scrollToSection(item.href)}
                className="text-foreground hover:text-primary block px-3 py-2 text-base font-medium transition-colors duration-200 w-full text-left rounded-md hover:bg-white/20"
              >
                {item.label}
              </button>
            ))}
